...
def run_one_batch_ICL(model_name, model, tokenizer, input_prompts, samples, file_paths, max_new_tokens=512):
    ...
    if 'Llama' in model_name:
        ...
        """
        if input_tokens.max().item() >= model.config.vocab_size:
            raise ValueError(f"Token ID {input_tokens.max().item()} exceeds vocab size {model.config.vocab_size}!")
        """
        try:
            ...
            print(f"[Error] Failed to generate for batch: {e}")
            return
    
    if "deepseek-v3" in model_name:
        client = OpenAI(api_key="", base_url="https://api.deepseek.com")#add your api key here
    ...
    if 'Qwen' in model_name:
        inputs = tokenizer(input_prompts, padding='longest', return_tensors="pt")
...
def run_one_batch_ICL_trans(model_name, model, tokenizer, input_prompts, samples, file_paths, max_new_tokens=512):
    '''
    Generate the completion for one batch of input prompts
    ...
    if "deepseek-r1" in model_name:
        ...
        for j in range(len(input_prompts)):
            ...
            try:
                ...
                prediction = response.choices[0].message.content.strip()
                cur_sample.update({'Timeline': prediction})
